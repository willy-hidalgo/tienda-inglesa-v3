...
class RLSForecastPipeline:
    ...
    def _run_section(
        ...
        df_oos = (
            self._aggregator.aggregate(raw_oos) if raw_oos.height else pl.DataFrame()
        )
        del raw_oos
        gc.collect()
        ...
        if df_oos.height:
            panel_parts.append(
                df_oos.with_columns(pl.lit("out_sample").alias("period_type"))
            )
        if df_fcst.height:
            panel_parts.append(
                df_fcst.with_columns(pl.lit("forecast_only").alias("period_type"))
            )
        panel_all = pl.concat(panel_parts, how="diagonal_relaxed").sort(
            ["unique_id", "ds"]
        )
        if meta:
        ...
        sec_coef_y = list(sec_model_y.final_coef_)
        sec_coef_p = list(sec_model_p.final_coef_)
        ...
        for store_id in stores:
            ...
            store_coefs_y[store_id] = list(m_y.final_coef_)
            store_coefs_p[store_id] = list(m_p.final_coef_)
        ...
        for store_id, m_y in store_models_y.items():
            ...
            pred = pred.select(["unique_id", "ds", "yhat", "valuehat"]).rename(
                {
        ...
        if store_pred_frames:
            ...
            preds_store_all = pl.DataFrame(schema={"unique_id": pl.Utf8, "ds": pl.Date})
        ...
        for store_id in stores:
            ...
            pred_store = preds.select(
                [
                    "unique_id",
                    "ds",
                    f"yhat_store_{store_id}",
                    f"valuehat_store_{store_id}",
                ]
            ).rename(
                {
        ...
        if store_err_frames:
            ...
            store_err_all = pl.DataFrame(schema={"unique_id": pl.Utf8})
            ...
            if col_abs in store_err_all.columns:
                ...
                store_wm_frames.append(
                    store_err_all.select(
                        [
                            "unique_id",
                            pl.col(col_abs).alias("sum_abs_err"),
                            pl.col(col_sum).alias("sum_err"),
                        ]
                    )
                    .with_columns(
                        [
        ...
        for store_id in store_ids:
            ...
            if wmape_col in metrics.columns:
                ...
                cond_better = (pl.col(wmape_col) < pl.col("wmape_sec")) | (
                    (pl.col(wmape_col) - pl.col("wmape_sec")).abs()
                    < 1e-9 & (pl.col(bias_col).abs() < pl.col("bias_sec").abs())
                )
                metrics = metrics.with_columns(
                    [
                        pl.when(cond_better)
        ...
        panel_all = panel_all.with_columns(
            pl.coalesce([pl.col("model_source"), pl.lit("section")]).alias(
                "model_source"
            )
        )
        ...
        # Section effects
        expr_y_sec = [pl.col(c) * coeff for c, coeff in zip(driver_cols, sec_coef_y)]
        ...
        # Prepare DF for SES
        resid_df = panel_all.select(["unique_id", "ds", "resid_y", "resid_v"])
        ...
        if getattr(settings, "COMPUTE_ROLLING_28", False):
            ...
            if roll.height and panel_all.height:
                ...
                if panel_all["ds"].dtype != roll["ds"].dtype:
                    roll = roll.with_columns(pl.col("ds").cast(panel_all["ds"].dtype))
                panel_all = panel_all.join(
