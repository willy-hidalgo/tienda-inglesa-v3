...
class RLSForecastRunner:
    ...
    def derive_sku_store_forecasts(
        ...
        la reconstrucción del pronóstico ocurre EN LOG-SPACE, y se separa en
        dos mecanismos distintos según el período:

          - In-sample (train): SIN SES. Se calculan dos pronósticos crudos
            por fila con los modelos finales (ajustados solo con train) de
            sección y de tienda — `yhat_seccion`/`yhat_tienda` y
            `valuehat_seccion`/`valuehat_tienda` — y, por cada unique_id de
            SKU+tienda, `_select_model_wmape` elige UN modelo ("seccion" o
            "tienda") minimizando el wMAPE in-sample de `y` (Σ|y−ŷ|/Σ|y|,
            filas y≠0), con empate resuelto por el BIAS más cercano a cero.
            La misma elección (`use_sto`) se usa para `yhat` y `valuehat`
            (no hay una selección independiente para el modelo de valor).
            El `yhat`/`valuehat` final de estas filas es la predicción cruda
            del modelo elegido: `_yhat_rls`/`_valuehat_rls` = expm1(X @
            coef_modelo_elegido), SIN corrección de residuo.

          - Out-sample / forecast-only: CON SES, usando el mismo modelo
            elegido arriba:
              yhat = expm1(intercept_modelo_elegido + efecto_modelo_elegido
                            + SES(log-residuo))
            donde `residuo_log = log1p(y) − (intercept + efecto)` del modelo
            elegido. `_apply_ses` calcula el SES (no causal, `ewm_mean` sin
            shift) sobre TODO el período con actuals (in_sample + out_sample)
            como una única serie continua, y propaga el último estado a
            forecast_only.

        Nota histórica: antes se restaba `efecto` (log-space) de `y`
        (lineal) para el residuo OOS, produciendo pronósticos 0/negativos en
        OOS/forecast_only (causa del ranking SKU+tienda vacío); ya corregido.
        ...
        for si, store_uid in enumerate(store_uids):
            ...
            if has_period:
                # in_sample: predicción CRUDA del modelo (sección o tienda) ya
                # elegido por `_select_model_wmape` (menor wMAPE in-sample de
                # y, empate → BIAS más cercano a cero) — SIN corrección SES.
                # out_sample / forecast_only: intercept + efecto del MISMO
                # modelo elegido + SES(log-residuo). `_y_neto_hat`/
                # `_v_neto_hat` están definidos sobre in_sample+out_sample
                # como una sola serie continua, pero solo se usan para estas
                # filas (ver `_select_model_wmape` / diseño confirmado).
