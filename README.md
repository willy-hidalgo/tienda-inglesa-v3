...
## Arquitectura de forecasting

El dominio de forecasting está separado en componentes cohesivos bajo `app/forecasting/`: configuración, calendario, features, agregación, métricas, panel/densificación, runner RLS y pipeline. `app/forecasts.py` actúa como fachada/entrypoint de compatibilidad.

El núcleo RLS se encuentra en `rls_opt/`, con el solver/API separado de los kernels numéricos Numba.

### Validación

La suite incluye tests de contrato RLS y tests unitarios de los componentes de forecasting. La ejecución completa requiere el runtime y dependencias declarados por el proyecto (`Python 3.14+` y `uv.lock`).
